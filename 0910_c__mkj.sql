...
--가상컬럼명을 설정
--제품테이블에서 제조업체의 중복데이터를 제거하고 검색하시오.(단 제조업체를 대표업체로 걸럼명 변경
...
--제품테이블에서 제품명, 단가를 검색하되, 단가에 500원을 더해서 조정 단가라는 가상 컬럼명으로 출력하시오.
...
--주문테이블에서 주문고객이 carrot이면서 수량이 15개 이상 주문한 주문제품, 수량 주문일자를 출력하시요.
select 주문제품, 수량, 주문일자
from 주문
...
-- 고객테이블에서 고객아이디에 p가 포함된 고객의 고객아이디, 고객이름울 출력하시오.
select 고객아이디, 고객이름
from 고객
where 고객아이디 like '%p%';
